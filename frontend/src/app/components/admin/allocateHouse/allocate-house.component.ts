...
export class AllocateHouseComponent implements OnInit {
  ...
  houseOptions: any[] = []; // Initialize an empty array for the options
  numberOfWings!: { name: number; value: number }; // Variable to bind to the selected value
  numberOfHouse!: { house_no: number; value: number }; // Variable to bind to the selected value
    ...
    this.userService.userSocietyId$.subscribe({
      next: (societyId) => {
        this.societyService.fetchSocietyData(societyId).subscribe({
          next: (response) => {
            const sortedWings = response.data.societyDetails.sort(
              (a: any, b: any) => a.name.localeCompare(b.name)
            );
            this.wingOptions = sortedWings.map((wing: any, index: number) => ({
              name: wing.name, // Use the wing name from the response
              value: wing.id, // Assign a value (index-based or from response)
            }));
          },
          error: (error) => {
    ...
    if (this.userDetailForm.valid && this.numberOfWings && this.numberOfHouse) {
      ...
      this.userService.addFamilyMember(allocationDetails).subscribe({
        next: (response: any) => {
          ...
          this.userDetailForm.reset(); // Reset the form fields
          this.numberOfWings = { name: 0, value: 0 }; // Reset to default empty values
          this.numberOfHouse = { house_no: 0, value: 0 }; // Reset to default empty values
        },
        error: (error: any) => {
